...
#        #parameters for control, default every 1/24 hours:
    ...
    def solve(self):
        ...
        if self.Control == False :
            u = 0
#            fc = 1
            C0 = [self.S0,  self.O0, self.X0,self.V0, self.T0]
...
class PlotCanvas(FigureCanvas):
    ...
    def plot(self,C,t):
        ...
        l4, = ax4.plot(time, data[:len(time),1]*1000, color = 'y',label = 'Oxygen')
...
#print(f.compounds())
#l = np.zeros(len(f.compounds()))
#for i in l:
#    label = []
#for i in range(len(f.compounds())):
#    f.compounds()[i]
#    label.append(f.compounds()[i])
#    print(label[i])
#    i+=1
#for count in range(len(modelInUse.compounds())):
#    'ax'+str(count)
#    print('ax'+str(count))
#    count += 1
#    'ax'+str(count).twinx()
#    
#l1, = ax.plot(time, data[:len(time),3], color = 'g',label='Ethanol')
